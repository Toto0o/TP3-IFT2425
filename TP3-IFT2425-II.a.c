...
float pairWiseSum(const float *arr, int size) 
{
  if (size == 1) {
    return arr[0];
  }
  int mid = size/2;
  float sum_left = pairWiseSum(arr, mid);
  float sum_right = pairWiseSum(arr + mid, size-mid);
...
//--------------------------------------------------------------------------------
 ...
 for (int i=0; i<=n; i++) {
  ...
  float pi = pairWiseSum(VctPts, NbInt+1);

  printf("pi = %.10f\n", pi);
  printf("err = %.10f", err(h, 1.0, 0.0, 1.0-h));
