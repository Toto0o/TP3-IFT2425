...
// author  : Antoine Tessier (20288121)
// date    : 09.04.2025
// version : 1.0
// language: C++
// note    :
...
//--------------------------------------------------------------------------------
 ...
 //Programmer ici
  ...
  int N = 10000000;
  double mu = 4.0;
  double initial_values[3] = {0.2, 0.4, 0.6};

  for (int idx=0; idx<3; idx++) {
    float x_float = initial_values[idx];
    double x_double = initial_values[idx];
    double sum_float = 0.0;
    double sum_double = 0.0;

    for (int i=0; i < N; i++) {
      x_float = mu*x_float*(1.0f-x_float);
      sum_float += sqrtf(x_float);
    }

    float pi_float = 2.0f / (sum_float/N);
    printf("Valeur initiale : %.1f | PI (float) = %.10f\n", initial_values[idx], pi_float);

    for (int i = 0; i < N; i++) {
      x_double = mu*x_double*(1.0f-x_double);
      sum_double += sqrtf(x_double);
    }

    double pi_double = 2.0 / (sum_double/N);
    printf("Valeur initiale : %.1f | PI (double) = %.10f\n\n", initial_values[idx], pi_double);
